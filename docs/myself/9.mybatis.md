...
name name = new name();
name.setname(“fred”);
int rows = mapper.insertname(name);// 完成后,id 已经被设置到对象中
...
```xml
<mapper namespace="com.lcb.mapping.userMapper"><!--association 一对一关联查询 -->
  <select id="getClass" parameterType="int"resultMap="ClassesResultMap">
select * from class c,teacher t where c.teacher_id=t.t_id and
c.c_id=#{id}</select>
 <resultMap type="com.lcb.user.Classes" id="ClassesResultMap">
   <!-- 实体类的字段名和数据表的字段名映射 -->
   <id property="id" column="c_id"/><result property="name" column="c_name"/><association property="teacher"
...
ofType="com.lcb.user.Student"><id property="id" column="s_id"/><result property="name" column="s_name"/></collection></resultMap>
</mapper>
```
...
### 1、mybatis的mapper接口没有实现类，其工作原理是什么？

### 2、如何开发mybatis分页插件，其原理是什么？

### 3、mybatis的一级缓存和二级缓存的区别？如何开启二级缓存？

### 4、mybatis如何获取自动生成的主键值？

### 5、mapper接口中传递多个参数有哪些方式？

### 6、mapper.xml文件中id是否可以重复？为什么？
